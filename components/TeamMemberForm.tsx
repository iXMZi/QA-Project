...
import { UserIcon, ClipboardDocumentListIcon, ChevronDownIcon, ChevronUpIcon, CheckCircleIcon, XCircleIcon, LightBulbIcon, BookmarkSquareIcon, ScaleIcon, UserGroupIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
export const TeamMemberForm: React.FC<TeamMemberFormProps> = ({ onSubmit, onSaveDraft, config, initialMemberName = '', initialReviewerName = '', initialEvaluationPeriod = '', initialStartDate = '', initialEndDate = '', initialStages, reviewers, currentUser, availableUnits = [] }) => {
  ...
  useEffect(() => {
    if (initialMemberName) setMemberName(initialMemberName);
    if (initialReviewerName) setReviewerName(initialReviewerName);
    if (initialStartDate) setStartDate(initialStartDate);
    if (initialEndDate) setEndDate(initialEndDate);
  }, [initialMemberName, initialReviewerName, initialStartDate, initialEndDate]);
  ...
  const getEvaluationData = (): AuditAnalysisResult => {
      ...
      return {
          auditType: `تقييم عضو فريق: ${memberName} - (${jobTitle}) ${evaluationPeriodStr ? `(${evaluationPeriodStr})` : ''} ${reviewerName ? `(المقيم: ${reviewerName})` : ''}`,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!memberName) {
        alert("يرجى إدخال اسم العضو قبل الإرسال.");
        return;
    }
  ...
  const getSubmitLabel = () => {
      if (currentUser?.role === 'مدير النظام') return 'اعتماد تقرير تقييم العضو';
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      <ConfirmModal 
        ...
        message="هل أنت متأكد من رغبتك في إرسال تقييم العضو للمراجعة؟ سيتم تجميد البيانات وإرسالها لصاحب الصلاحية."
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">بيانات تقييم عضو الفريق</h2>
            <p className="text-sm text-slate-500">تقييم الأداء الفني والمهني للمدقق في المهام الرقابية</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-2">اسم العضو</label>
              <input
                type="text"
                value={memberName}
                onChange={(e) => setMemberName(e.target.value)}
                className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-slate-800 font-bold"
                placeholder="الاسم الثلاثي..."
              />
            </div>
            <div>
               <label className="block text-sm font-semibold text-slate-700 mb-2">المسمى الوظيفي / الدور</label>
               <input 
                  type="text"
                  value={jobTitle}
                  onChange={(e) => setJobTitle(e.target.value)}
                  className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-slate-800 font-bold"
                  placeholder="مثال: مدقق رئيسي، مدقق مساعد..."
               />
            </div>
            <div>
                ...
                <select
                    ...
                    className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-slate-800 bg-slate-50"
                >
             ...
             <div className="grid grid-cols-2 gap-4">
                 <div>
                     ...
                     <input
                        ...
                        className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none text-slate-800"
                    />
                 </div>
                 <div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">ملخص تقييم الأداء العام</label>
            <textarea
              value={summary}
              onChange={(e) => setSummary(e.target.value)}
              className="w-full h-28 p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none resize-none text-slate-800"
              placeholder="اكتب رأيك العام في أداء العضو، نقاط التميز، ومجالات التحسين..."
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
           ...
           <h3 className="text-lg font-bold text-slate-800">عناصر التقييم الفني</h3>
        </div>
          ...
          <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <button
              ...
              className={`w-full flex items-center justify-between p-4 transition-colors ${
                expandedStage === index ? 'bg-teal-50/50' : 'hover:bg-slate-50'
              }`}
            >
              ...
              <div className="p-6 border-t border-slate-100 space-y-6 bg-white animate-fade-in">
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                   <div className="bg-slate-50 px-4 py-2 border-b border-slate-200">
                     <h4 className="font-bold text-sm text-slate-700">تقييم الأداء التفصيلي</h4>
                   </div>
                   <div className="divide-y divide-slate-100">
                     {stage.criteriaResults.map((criterion, cIndex) => (
                        <div key={cIndex} className="p-5 hover:bg-slate-50/50 transition-colors">
                           ...
                           <div className="pl-2 border-r-2 border-slate-200 pr-4 space-y-3 bg-teal-50/30 p-3 rounded-lg">
                              <div>
                                  ...
                                  <textarea 
                                    ...
                                    className="w-full p-2 text-xs border border-slate-300 rounded bg-white h-16 focus:ring-1 focus:ring-teal-500 outline-none"
                                  />
                              </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-200">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        <textarea
                          ...
                          className="w-full h-20 p-3 border border-slate-300 rounded-lg focus:ring-1 focus:ring-teal-500 outline-none text-sm"
      ...
      <div className="pt-4 flex gap-3">
        ...
        <button
          ...
          className="flex-1 bg-teal-700 hover:bg-teal-800 text-white font-bold py-4 px-6 rounded-xl shadow-lg transition-all transform hover:scale-[1.01] flex items-center justify-center gap-2"
        >
