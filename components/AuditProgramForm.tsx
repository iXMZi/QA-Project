...
import { DocumentTextIcon, ClipboardDocumentListIcon, ChevronDownIcon, ChevronUpIcon, CheckCircleIcon, XCircleIcon, LightBulbIcon, BookmarkSquareIcon, ScaleIcon, UserGroupIcon, XMarkIcon, BuildingOffice2Icon } from '@heroicons/react/24/outline';
...
export const AuditProgramForm: React.FC<AuditProgramFormProps> = ({ onSubmit, onSaveDraft, config, initialProgramName = '', initialReviewerName = '', initialEvaluationPeriod = '', initialStartDate = '', initialEndDate = '', initialStages, reviewers, currentUser, availableUnits = [] }) => {
  ...
  }, [initialProgramName, initialReviewerName, initialStartDate, initialEndDate]);
  ...
  const getEvaluationData = (): AuditAnalysisResult => {
      ...
      const targetDisplay = associatedMission ? `${programName} (مرتبط بـ: ${associatedMission})` : programName;

      return {
          auditType: `${auditTypeLabel} - تقييم برنامج رقابي ${evaluationPeriodStr ? `(${evaluationPeriodStr})` : ''} ${reviewerName ? `(المقيم: ${reviewerName})` : ''}`,
          targetName: targetDisplay,
          targetType: 'AuditProgram',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!programName) {
        alert("يرجى إدخال اسم البرنامج الرقابي قبل الإرسال.");
        return;
    }
  ...
  const getSubmitLabel = () => {
      if (currentUser?.role === 'مدير النظام') return 'اعتماد تقييم البرنامج';
  ...
  return (
    <div className="space-y-8 animate-fade-in font-sans">
      <ConfirmModal 
        ...
        message="هل أنت متأكد من رغبتك في إرسال تقييم البرنامج الرقابي للمراجعة؟"
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">تقييم برنامج رقابي</h2>
            <p className="text-sm text-slate-500">تقييم جودة إعداد وتصميم البرامج الرقابية</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-2">اسم البرنامج الرقابي</label>
              <input
                type="text"
                value={programName}
                onChange={(e) => setProgramName(e.target.value)}
                className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none text-slate-800 font-bold"
                placeholder="عنوان البرنامج..."
              />
            </div>
            <div>
               <label className="block text-sm font-semibold text-slate-700 mb-2">المهمة المرتبطة (اختياري)</label>
               <input 
                  type="text"
                  value={associatedMission}
                  onChange={(e) => setAssociatedMission(e.target.value)}
                  className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none text-slate-800"
                  placeholder="اسم المهمة إن وجد..."
               />
            </div>
            <div>
               <label className="block text-sm font-semibold text-slate-700 mb-2">نوع التدقيق</label>
               <select 
                  value={auditType}
                  onChange={(e) => setAuditType(e.target.value as AuditType)}
                  className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none bg-white font-bold"
               >
                 ...
                 <option value={AuditType.PROFESSIONAL_REVIEW}>مراجعة جودة الأداء المهني</option>
               </select>
            </div>
            <div>
                ...
                <select
                    ...
                    className="w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none text-slate-800 bg-slate-50"
                >
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">ملخص تقييم جودة البرنامج</label>
            <textarea
              value={summary}
              onChange={(e) => setSummary(e.target.value)}
              className="w-full h-28 p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 outline-none resize-none text-slate-800"
              placeholder="اكتب ملخصاً عاماً حول جودة تصميم البرنامج وتغطيته للأهداف..."
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 mb-2">
           ...
           <h3 className="text-lg font-bold text-slate-800">معايير تقييم البرنامج</h3>
        </div>
          ...
          <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <button
              ...
              className={`w-full flex items-center justify-between p-4 transition-colors ${
                expandedStage === index ? 'bg-cyan-50/50' : 'hover:bg-slate-50'
              }`}
            >
              <div className="flex items-center gap-3">
                <span className="flex items-center justify-center w-8 h-8 rounded-full bg-cyan-100 text-cyan-700 font-bold text-sm">
                  {index + 1}
                </span>
              ...
              <div className="p-6 border-t border-slate-100 space-y-6 bg-white animate-fade-in">
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden">
                   <div className="bg-slate-50 px-4 py-2 border-b border-slate-200">
                     <h4 className="font-bold text-sm text-slate-700">التحقق من المعايير</h4>
                   </div>
                   <div className="divide-y divide-slate-100">
                     {stage.criteriaResults.map((criterion, cIndex) => (
                        <div key={cIndex} className="p-5 hover:bg-slate-50/50 transition-colors">
                           ...
                           <div className="pl-2 border-r-2 border-slate-200 pr-4 space-y-3 bg-cyan-50/30 p-3 rounded-lg">
                              <div>
                                  <label className="block text-xs font-semibold text-slate-600 mb-1">التحليل / الملاحظات</label>
                                  <textarea 
                                    ...
                                    className="w-full p-2 text-xs border border-slate-300 rounded bg-white h-16 focus:ring-1 focus:ring-cyan-500 outline-none"
                                  />
                              </div>
                              
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  <div>
                                    <label className="block text-xs font-semibold text-amber-700 mb-1">أوجه القصور</label>
                                    <textarea 
                                  ...
                                  <div className="relative">
                                    ...
                                    >
                                        {criterion.responsibleUnits ? (
                                            ...
                                            <span className="text-slate-400">اختر الوحدات...</span>
                                        )}
                                    </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-200">
                    <h4 className="text-sm font-bold text-slate-700 mb-4">تعليقات عامة</h4>
                    <div className="grid grid-cols-1 gap-4">
                        <textarea
                          value={stage.analysis}
                          onChange={(e) => handleStageChange(index, 'analysis', e.target.value)}
                          className="w-full h-20 p-3 border border-slate-300 rounded-lg focus:ring-1 focus:ring-cyan-500 outline-none text-sm"
      ...
      <div className="pt-4 flex gap-3">
        ...
        <button
          ...
          className="flex-1 bg-cyan-700 hover:bg-cyan-800 text-white font-bold py-4 px-6 rounded-xl shadow-lg transition-all transform hover:scale-[1.01] flex items-center justify-center gap-2"
        >
